...
stopwords = stopwords.words("vietnamese")
...
def tokenize_reviews(reviews):
  ...
  reviews = tf.strings.regex_replace(reviews, r"[áàảãạăắằẳẵặâấầẩẫậ]", "a")
  reviews = tf.strings.regex_replace(reviews, r"[éèẻẽẹêếềểễệ]", "e")
  reviews = tf.strings.regex_replace(reviews, r"[íìỉĩị]", "i")
  reviews = tf.strings.regex_replace(reviews, r"[óòỏõọôốồổỗộơớờởỡợ]", "o")
  reviews = tf.strings.regex_replace(reviews, r"[úùủũụưứừửữự]", "u")
  reviews = tf.strings.regex_replace(reviews, r"[ýỳỷỹỵ]", "y")
  reviews = tf.strings.regex_replace(reviews, r"[đ]", "d")
